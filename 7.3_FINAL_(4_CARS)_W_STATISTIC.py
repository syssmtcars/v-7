...
Label(root, text='Cross ').grid(column=0, row=0)
Label(root, text='QOC ').grid(column=1, row=0)
Label(root, text='[s,  f,  l,  r]').grid(column=2, row=0)
for i in range(10):
    Label(root, text='   |').grid(column=4, row=i)
    Label(root, text='|   ').grid(column=5, row=i)

number_of_cross=0
for i in range(len(cross_letter)):
    for j in range(1, 4):
        cross_name = cross_letter[i] + str(j)
        Label(root, text=cross_name).grid(column=0, row=number_of_cross + 1)
        number_of_cross+=1

Label(root, text='Unit').grid(column=6, row=0)
Label(root, text='Yellow').grid(column=6, row=1)
Label(root, text='Blue').grid(column=6, row=2)
Label(root, text='Red').grid(column=6, row=3)
Label(root, text='Purple').grid(column=6, row=4)

Label(root, text='Cross').grid(column=7, row=0)
Label(root, text='Itenerary').grid(column=8, row=0)
Label(root, text='Motion').grid(column=9, row=0)
Label(root, text='Next Cross').grid(column=10, row=0)
Label(root, text='Global_MSG').grid(column=0, row=9)
...
def main_program():
    ...
    # cap.set(cv2.CAP_PROP_CONTRAST, 0.75)
    ...
    #            a1        a2        a3     b1     b2     b3
    m_unit_yellow = [['l', 'b3'], ['l', 'a3'], ['f', 'a2'], ['l', 'a1'], ['l', 'b1'], ['l', 'b2'], ['r', 'a2'],
                     ['r', 'a3'], ['r', 'b3'], ['f', 'b2'], ['r', 'b1'], ['r', 'a1'], ['r', 'a2'], ['l', 'b2'],
                     ['cycle', 'b2']]  # m_unit_1
    # m_unit_yellow = ['none','left','left','none','left','left'] #a1 to b3
    m_unit_blue = [['r', 'b1'], ['r', 'a1'], ['r', 'a2'], ['l', 'b2'], ['l', 'b3'], ['l', 'a3'], ['l', 'a2'],
                   ['r', 'b2'], ['cycle', 'b1']]
    m_unit_red = [['r', 'b2'], ['r', 'a2'], ['r', 'a3'], ['r', 'b3'], ['cycle', 'b2']]
    m_unit_purple = m_unit_1
    ...
    while True:
        ...
        for cnt_yellow in contours0_yellow:
            ...
            if area > 100:
                ...
                elif m_unit_yellow[n_yellow + 1][0] == 'stop':
                    #print('stop')
                    global_msg_yellow = 's'
                ...
                elif cross_yellow == 'notcross_yellow':
        ...
        for cnt_purple in contours0_purple:
            ...
            if area > 100:
                ...
                if cross_purple == m_unit_purple[n_purple][1]:
                    ...
                    #print('forward')
                 # print({'cross_purple': str(cross_purple), 'old_cross_purple': str(old_cross_purple)})
    ...
    # Логика разъезда на перекрестках
        ...
        for i in range(len(cross_letter)):
            for j in range(1, 4):
                ...
                #purpleqq logic
                if order_in_cross.find('PP')>=5:
                    decision_var = order_in_cross[3] + order_in_cross[8]
                    # print(decision_var)
                    if decision_var == 'lr' or decision_var == 'fr' or decision_var == 'rl':
                        if m_unit_purple[n_purple + 1][1] == m_unit_yellow[n_yellow + 1][1] or m_unit_purple[n_purple + 1][
                            1] == m_unit_red[n_red + 1][1] or m_unit_purple[n_purple + 1][1] == m_unit_blue[n_blue + 1][1] :  # Т.е.следующие cross совпадают
                            global_msg_purple = 's'
                            CASE_SUBJECT = decision_var
                    ...
                    if decision_var == 'ff':
                        if m_unit_purple[n_purple][1][1] != m_unit_purple[n_purple + 1][1][1] and m_unit_yellow[n_yellow][1][
                            1] != m_unit_yellow[n_yellow + 1][1][1]:  # Т.е.следующие letter of cross совпадают
                            global_msg_purple = 's'
                            CASE_SUBJECT = decision_var

                        if m_unit_purple[n_purple][1][1] != m_unit_purple[n_purple + 1][1][1] and m_unit_blue[n_blue][1][
                            1] != m_unit_blue[n_blue + 1][1][1]:  # Т.е.следующие letter of cross совпадают
                            global_msg_purple = 's'
                            CASE_SUBJECT = decision_var
        ...
        if flag_who_is_first != old_who_is_first:
            ...
            for i in range(len(cross_letter)):
                for j in range(1, 4):
                    ...
                    Label(root,text = str(cross_counter_of_cars[number_of_cross])).grid(column = 1 , row = number_of_cross+1)
                    Label(root, text=str(cross_counter_of_motion[number_of_cross])).grid(column=2, row=number_of_cross + 1)
            ...
            Label(root, text=global_msg).grid(column=1, row=9)

            Label(root, text=str(cross_yellow)).grid(column=7, row=1)
            Label(root, text=m_unit_yellow[n_yellow][1]).grid(column=8, row=1)
            Label(root, text=m_unit_yellow[n_yellow][0]).grid(column=9, row=1)
            Label(root, text=m_unit_yellow[n_yellow + 1][1]).grid(column=10, row=1)

            Label(root, text=str(cross_blue)).grid(column=7, row=2)
            Label(root, text=m_unit_blue[n_blue][1]).grid(column=8, row=2)
            Label(root, text=m_unit_blue[n_blue][0]).grid(column=9, row=2)
            Label(root, text=m_unit_blue[n_blue + 1][1]).grid(column=10, row=2)

            Label(root, text=str(cross_red)).grid(column=7, row=3)
            Label(root, text=m_unit_red[n_red][1]).grid(column=8, row=3)
            Label(root, text=m_unit_red[n_red][0]).grid(column=9, row=3)
            Label(root, text=m_unit_red[n_red + 1][1]).grid(column=10, row=3)

            Label(root, text=str(cross_purple)).grid(column=7, row=4)
            Label(root, text=m_unit_blue[n_blue][1]).grid(column=8, row=4)
            Label(root, text=m_unit_blue[n_blue][0]).grid(column=9, row=4)
            Label(root, text=m_unit_blue[n_blue + 1][1]).grid(column=10, row=4)




            # print(number_of_cross)
        ...
        global_shutdown_while = 1
        if global_shutdown_while == 1:
            break
