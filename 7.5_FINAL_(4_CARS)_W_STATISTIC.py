...
# Логика разъезда на перекрестках
    ...
    for i in range(len(cross_letter)):
        for j in range(1, 4):
            ...
            if cross_yellow=='notcross'or cross_blue=='notcross' or cross_red=='notcross'or cross_purple=='notcross':

                if m_unit_yellow[n_yellow] == m_unit_blue[n_blue+1]:
                    global_msg_blue='s'
                if m_unit_yellow[n_yellow] == m_unit_red[n_red+1]:
                    global_msg_red='s'
                if  m_unit_yellow[n_yellow] == m_unit_purple[n_purple+1]:
                    global_msg_purple='s'

                if m_unit_blue[n_blue] == m_unit_yellow[n_yellow+1]:
                    global_msg_yellow='s'
                if m_unit_blue[n_blue] == m_unit_red[n_red+1]:
                    global_msg_red='s'
                if  m_unit_blue[n_blue] == m_unit_purple[n_purple+1]:
                    global_msg_purple='s'

                if m_unit_red[n_red] == m_unit_yellow[n_yellow+1]:
                       global_msg_yellow='s'
                if m_unit_red[n_red] == m_unit_blue[n_blue+1]:
                       global_msg_blue='s'
                if  m_unit_red[n_red] == m_unit_purple[n_purple+1]:
                       global_msg_purple='s'

                if m_unit_purple[n_purple] == m_unit_yellow[n_yellow+1]:
                       global_msg_yellow='s'
                if m_unit_purple[n_purple] == m_unit_blue[n_blue+1]:
                       global_msg_blue='s'
                if  m_unit_purple[n_purple] == m_unit_red[n_red+1]:
                       global_msg_red='s'
